...
export class FirstTimeSetupComponent extends Container {
	...
	private update(): void {
		...
		this.addChild(
			new Text(
				theme.fg("accent", theme.bold(t("Welcome to {app}, the minimal coding agent.", { app: APP_NAME }))),
				1,
				0,
			),
		);
		...
		this.addChild(
			new Text(
				rawKeyHint("↑↓", t("navigate")) +
					"  " +
					keyHint("tui.select.confirm", t(this.step === "theme" ? "continue" : "finish")) +
					"  " +
					keyHint("tui.select.cancel", t("skip setup")),
				1,
				0,
			),
		);
